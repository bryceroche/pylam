select
    ...
    `DEVTRUGREENV2`.`HOURS`.`USER_ID` AS `USER_ID`,
    `DEVTRUGREENV2`.`HOURS`.`WEEKDAY` AS `WEEKDAY`,
    `DEVTRUGREENV2`.`HOURS`.`WEEKDAY_ID` AS `WEEKDAY_ID`,
    concat( `C`.`THE_DATE`, ' ', `DEVTRUGREENV2`.`HOURS`.`START_TIME` ) AS `AVAILABLE_START`,
    concat( `C`.`THE_DATE`, ' ', `DEVTRUGREENV2`.`HOURS`.`END_TIME` ) AS `AVAILABLE_END`,
    concat( `C`.`THE_DATE`, ' ', `DEVTRUGREENV2`.`HOURS`.`LUNCH_START` ) AS `LUNCH_START`,
    concat( `C`.`THE_DATE`, ' ', `DEVTRUGREENV2`.`HOURS`.`LUNCH_END` ) AS `LUNCH_END`
from
    (
        `DEVTRUGREENV2`.`HOURS` join `DEVTRUGREENV2`.`V_DAYOFMONTH` `C` on
        (
            (
                `C`.`WEEK_DAY_ID` = `DEVTRUGREENV2`.`HOURS`.`WEEKDAY_ID`
            )
        )
    )

    
CREATE DEFINER=`adritrugreendba`@`%` PROCEDURE `TRUGREEN`.`SP_MONTH_AVAIL`(IN P_USER_ID INT, IN INT_MINUTE_BREAK INT)
BEGIN
